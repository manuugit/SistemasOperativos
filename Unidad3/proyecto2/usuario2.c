...
int main(void) {
    ...
    //cola para recibir (cola 1)
    if ((key1 = ftok("cola1.txt", 'B')) == -1) {
        perror("ftok");
        exit(1);
    }
    
    if ((msqid1 = msgget(key1, PERMS)) == -1) { /* se conecta a la cola 1 */
        perror("msgget");
        exit(1);
    }
    ...
    pthread_create(&hiloEnvia, NULL, &enviarMensaje, msqid2);
    pthread_create(&hiloRecibe, NULL, &recibirMensaje,msqid1);
